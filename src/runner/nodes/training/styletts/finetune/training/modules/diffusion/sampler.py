...
class RK4Sampler(Sampler):
    ...
    def derivative(self, x: Tensor, fn: Callable, sigma: float) -> Tensor:
        return (x - fn(x, sigma=sigma)) / sigma

    def step(self, x: Tensor, fn: Callable, sigma: float, sigma_next: float) -> Tensor:
        h = sigma_next - sigma
        sigma_mid = sigma + 0.5 * h

        k1 = self.derivative(x, fn=fn, sigma=sigma)
        k2 = self.derivative(x + 0.5 * h * k1, fn=fn, sigma=sigma_mid)
        k3 = self.derivative(x + 0.5 * h * k2, fn=fn, sigma=sigma_mid)
        k4 = self.derivative(x + h * k3, fn=fn, sigma=sigma_next)
