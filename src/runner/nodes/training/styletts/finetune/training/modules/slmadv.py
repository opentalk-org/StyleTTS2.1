...
class SLMAdversarialLoss(torch.nn.Module):
    ...
    def __init__(self, model, wl, sampler, min_len, max_len, batch_max_samples, skip_update=10, sig=1.5):
        ...
        self.batch_max_samples = int(batch_max_samples)
    ...
    def forward(self, iters, y_rec_gt, y_rec_gt_pred, waves, mel_input_length, ref_text, ref_lengths, use_ind, s_trg, ref_s=None):
        ...
        with profiling_fn("bert_encoding"):
        ...
        if use_ind and np.random.rand() < 0.5:
            ...
            with profiling_fn("diffusion_sampling"):
        ...
        with profiling_fn("duration_prediction"):
        ...
        with profiling_fn("alignment_construction"):
        ...
        with profiling_fn("prosody_prediction"):
        ...
        with profiling_fn("crop_collection"):
            for bib in range(len(output_lengths)):
                ...
                if self.batch_max_samples > 0 and len(wav) >= self.batch_max_samples:
                    break
        ...
        with profiling_fn("pitch_and_decoder"):
            F0_fake, N_fake = self.model.predictor(p_en, sp[:, 128:])
        ...
        with profiling_fn("wavlm_generator"):
            gen_loss = self.wl.generator(y_pred.squeeze())

        gen_loss = gen_loss.mean()

        return d_loss, gen_loss, y_pred.detach().float().cpu().numpy()
