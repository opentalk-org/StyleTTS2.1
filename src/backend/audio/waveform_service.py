...
class WaveformService:
    ...
    async def ensure(self, audio_file_id: uuid.UUID) -> str:
    ...
    async def _generate(self, audio_file_id: uuid.UUID) -> None:
        loop = asyncio.get_running_loop()
        try:
            audio_bytes, duration = await loop.run_in_executor(None, self._read_audio, audio_file_id)
            waveform = await loop.run_in_executor(self._get_pool(), waveform_from_audio_bytes, audio_bytes)
            await loop.run_in_executor(None, self._write_waveform, audio_file_id, duration, waveform)
        except Exception:
            logger.exception("waveform generation failed audio_file_id=%s", audio_file_id)
            async with self._lock:
    ...
    def _get_pool(self) -> ProcessPoolExecutor:
        if self._pool is None:
            self._pool = ProcessPoolExecutor(max_workers=self._max_workers, mp_context=multiprocessing.get_context("spawn"))
        return self._pool

    def _exists(self, audio_file_id: uuid.UUID) -> bool:
        with database_session() as session:
            return session.get(AudioWaveform, audio_file_id) is not None

    def _read_audio(self, audio_file_id: uuid.UUID) -> tuple[bytes, float]:
        with database_session() as session:
            item = audio_crud.get_audio_file(session, audio_file_id)
            data = audio_crud.read_audio_file(session, audio_file_id)
            return data, item.duration

    def _write_waveform(self, audio_file_id: uuid.UUID, duration: float, waveform) -> None:
        with database_session() as session:
