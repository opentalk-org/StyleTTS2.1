...
class ValidationCandidates:
    ...
    voice_by_audio_file_id: dict[UUID, str]

    def voice_for(self, audio_file_id: UUID) -> str:
        return self.voice_by_audio_file_id[audio_file_id]
...
def build_validation_candidates(
    ...
    return ValidationCandidates(
        tuple(
            sorted(
                audio_id
                for audio_ids in by_voice.values()
        ...
        {
            audio_id: voice_id
            for voice_id, audio_ids in by_voice.items()
            for audio_id in audio_ids
        },
    )
