...
from runner.nodes.assets.catalog_runtime.http import (
    download_url_bytes,
...
def ensure_checkpoint_bundle(
    spec: CheckpointSpec, *, logger: logging.Logger | None = None
    ...
    with database_session() as session:
        ...
        if existing is not None:
            try:
                ...
                if spec.is_valid(path):
                    log.info(
                        "catalog checkpoint cached key=%s name=%s (reusing existing)",
                        spec.key,
                        spec.name,
                    )
                    ...
                    updated = asset_crud.update_checkpoint(
                        ...
                        CheckpointUpdate(
                            name=spec.name,
                            folder_path=None,
                            type_=spec.type_.value,
                            metadata=metadata,
                        ),
                    )
                    return updated, True
        log.info(
            "catalog checkpoint download starting key=%s name=%s files=%s",
            spec.key,
            spec.name,
            len(spec.files),
        )
        with TemporaryDirectory(prefix=f"runflow-catalog-{spec.key}-") as tmp:
            ...
            log.info(
                "catalog checkpoint download complete key=%s name=%s",
                spec.key,
                spec.name,
            )
...
def ensure_extra_file(
    spec: ExtraFileSpec, *, logger: logging.Logger | None = None
) -> tuple[AssetRecord, bool]:
    log = logger or _LOGGER
    with database_session() as session:
        ...
        if existing is not None:
            ...
            if path.read_bytes().strip():
                log.info(
                    "catalog extra file cached key=%s name=%s (reusing existing)",
                    spec.key,
                    spec.name,
                )
                ...
                return updated, True
        log.info(
            "catalog extra file download starting key=%s name=%s", spec.key, spec.name
        )
        data = download_url_bytes(
            spec.url, error_prefix=f"{spec.key}_download_failed", logger=log
        )
        if not data.strip():
        ...
        if existing is None:
            created = asset_crud.create_extra_file(
                session,
                ExtraFileCreate(
                    name=spec.name, data=data, type_=spec.type_.value, metadata=metadata
                ),
            )
            return created, False
        updated = asset_crud.update_extra_file(
...
def checkpoint_payload(
    item: AssetRecord, *, skipped: bool, filename: str | None = None
) -> dict[str, Any]:
    payload = {
...
def extra_file_payload(item: AssetRecord, *, skipped: bool) -> dict[str, Any]:
    return {
...
def _download_checkpoint_files(
    spec: CheckpointSpec, folder: Path, logger: logging.Logger
) -> None:
    total = len(spec.files)
    for index, file in enumerate(spec.files, start=1):
        logger.info(
            "catalog file download key=%s file=%s index=%s/%s",
            spec.key,
            file.name,
            index,
            total,
        ...
        download_url_to_file(
            file.url,
            folder / file.name,
            error_prefix=f"{spec.key}_download_failed",
            logger=logger,
        )


def _checkpoint_metadata(
    spec: CheckpointSpec, folder: Path, current: dict[str, Any]
) -> dict[str, Any]:
    return {
        **current,
        **spec.metadata,
        **spec.metadata_from_path(folder),
