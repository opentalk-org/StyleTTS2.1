...
The flake builds two images. The **hub** (`nix build .#image`, entrypoint `nix/entrypoint.sh`) runs the full stack (Postgres, PgBouncer, NATS, RustFS, backend, frontend, Trackio, a runner) and is stateful — run it on a persistent GPU box (Vast.ai). The **runner** (`nix build .#runner-image`, entrypoint `nix/runner-entrypoint.sh`) is a slim, self-contained GPU worker (baked ML venv + tailscale + proxychains); run it on any GPU host and it reaches the hub's DB/NATS/S3 as a client over a self-hosted Headscale tailnet.
