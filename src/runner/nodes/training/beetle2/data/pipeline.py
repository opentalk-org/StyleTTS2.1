...
@dataclass(frozen=True)
class _QueuedWindow:
    batches: tuple[_QueuedBatch | _FailedBatch, ...]
...
class TrainingDataPipeline:
    def __init__(
        ...
        self._active: list[_QueuedBatch | _FailedBatch] = []
        self._in_flight: _QueuedBatch | _FailedBatch | None = None
    ...
    def next_batch(self) -> TrainingBatch:
        ...
        if isinstance(self._in_flight, _FailedBatch):
            raise BatchLoadError(str(self._in_flight.error)) from self._in_flight.error
        ...
        try:
            while not self._stop.is_set():
                ...
                for item, decoded_bytes in zip(plans, estimates, strict=True):
                    try:
                        prepared = self._prepare_batch(item, decoded_bytes)
                    except Exception as error:
                        prepared = _FailedBatch(
                            error,
                            item.state_after,
                            decoded_bytes,
                        )
                    queued.append(prepared)
                if not self._put(_QueuedWindow(tuple(queued))):
