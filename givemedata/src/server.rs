...
use crate::symbols::{TextCleaner, boundary_token_id, text_to_tensor};
    ...
    while let Some(req) = req_stream.message().await? {
        ...
        if req.split() == Split::Validation {
            ...
            for sample in batch {
                ...
                let mut text = BytesMut::with_capacity(8 * sample.text.len());
...
impl GiveMeDataService for GiveMeData {
    async fn init(&self, request: Request<InitRequest>) -> Result<Response<InitResponse>, Status> {
        ...
        println!("initializing session {session_id}");
        ...
        println!(
            ...
            request.validation_samples, request.dataset_id
        );
