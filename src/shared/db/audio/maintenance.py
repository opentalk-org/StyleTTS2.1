...
def repack_legacy_audio_packs(
    ...
    items = _repack_live_audio_files(session, selected)
    source_packs = {
        pack.id: resolved_store.download(pack.path)
        for pack in selected
    }
    source_audio = {
        item.id: _slice_from_pack(source_packs[item.bucket_file_id], item)
        for item in items
    }
    writer = AudioPackWriter(session, resolved_store, config)
    writes = {
        item.id: writer.append(source_audio[item.id])
        for item in items
    }
    writer.flush()
    for item in items:
    ...
    session.flush()
    created = {
        write.bucket_file.id: write.bucket_file
    ...
    for item in items:
        ...
        assert replacement[start:end] == source_audio[item.id], (
            f"repack byte mismatch: {item.id}"
        )
...
def _legacy_packs(
    ...
    return sorted(
        (
            pack
            for pack in packs
            if _is_legacy_path(pack.path, prefix)
        ),
