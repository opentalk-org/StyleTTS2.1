...
- Torch and other project Python libraries are available through the project interpreter: use `nix develop --command python -m <module> ...` or `nix develop --command python <script.py>`. Do not invoke a bare Python tool entry point through `nix develop`, because `PATH` may resolve a host executable using a different interpreter without the project libraries. Verify the environment with `nix develop --command python -c 'import torch; print(torch.__version__)'`.
